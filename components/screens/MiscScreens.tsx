  ...
  function NotifRow({ id, label, meta, last }: { id: string; label: string; meta: string; last?: boolean }) {
    ...
    return (
      <div className="list-row" style={{ borderBottom: last ? 'none' : '1px solid var(--outline-variant)', borderTop: 'none' }}>
        ...
        <button onClick={() => toggleNotif(id)} className={`toggle${on ? ' is-on' : ''}`} role="switch" aria-checked={on} aria-label={`Ativar notificações para ${label}`}><span className="thumb" aria-hidden="true" /></button>
      </div>
    );
  }
        ...
        <div className="card-filled">
          {pushSupported && (
            <div className="list-row" style={{ borderBottom: '1px solid var(--outline-variant)', borderTop: 'none' }}>
              ...
              <button onClick={togglePush} disabled={pushBusy} className={`toggle${pushOn ? ' is-on' : ''}`} style={{ opacity: pushBusy ? 0.6 : 1 }} role="switch" aria-checked={pushOn} aria-label="Notificações no dispositivo"><span className="thumb" aria-hidden="true" /></button>
            </div>
          )}
