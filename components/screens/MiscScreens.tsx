  ...
  function NotifRow({ id, label, meta, last }: { id: string; label: string; meta: string; last?: boolean }) {
    ...
    return (
      <div className="list-row" style={{ borderBottom: last ? 'none' : '1px solid var(--outline-variant)', borderTop: 'none' }}>
        ...
        <button role="switch" aria-checked={on} aria-label={label} onClick={() => toggleNotif(id)} className={`toggle${on ? ' is-on' : ''}`}><span className="thumb" /></button>
      </div>
    );
  }
        ...
        <div className="card-filled">
          {pushSupported && (
            <div className="list-row" style={{ borderBottom: '1px solid var(--outline-variant)', borderTop: 'none' }}>
              ...
              <button role="switch" aria-checked={pushOn} aria-label="Notificações no aparelho" onClick={togglePush} disabled={pushBusy} className={`toggle${pushOn ? ' is-on' : ''}`} style={{ opacity: pushBusy ? 0.6 : 1 }}><span className="thumb" /></button>
            </div>
          )}
